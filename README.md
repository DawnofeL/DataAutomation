...
| `max_retry` | 不合格重发几次 | 重发一次的钱等于跑一次。默认 3 |
...
落在区间外的段会被打回重写，重发 `max_retry` 次还不行就进 `rejected_*.json`。
...
落盘前有一道闸门，逐段判合格不合格，分两个文件落：

| 文件 | 装什么 |
|---|---|
| `dialogues_<domain>.json` | 合格的。这个文件里永远 0 硬失败 |
| `rejected_<domain>.json` | 打回的，每段带 `problems` 说明哪一条不合格 |

闸门两关：结构（轮数落在区间里、角色严格交替）和 `check.py` 的硬失败
（禁词、问句结尾、占位符、语气词超限等等）。两关都过才进 `dialogues`。

模型第一次没写好会被打回重写，最多 `max_retry` 次。重发用光还不合格的，
逐段分流，合格的那几段照样留下，不连累整批。
...
```
  ...
  ┌─ 落盘前分流 ──────────────────────────────────────────────────────────
  │ 两性关系  合格 13 段  打回 2 段
  │   4 轮 3 段 · 5 轮 6 段 · 6 轮 2 段 · 7 轮 2 段
  │   彩礼/1-5  第2条  问句结尾
  │   约会/2-3  第8条  占位符 XX
  └
...
│   ├── test_wordlists.py         # 核对 check.py 的词表和 words.md 对不对得上
│   └── ui/                       # 命令行长什么样，全在这个包里
│       ├── theme.py              # 宽度、线条、颜色
...
3. `references/words.md`：用词红线。五部分，前三部分命中即不合格：禁词（网络烂梗、过度夸张、虚假共鸣、口语糟粕、拟声词、后缀句式、收尾套话）、企业黑话（绝对禁用 27 个 + 看语境 14 个）、AI 句式（翻案腔、名词化、模型路标、硬停词、抒情词、三连排比、标点）。第四部分讲笑怎么写：连着打三个以上的哈，一段里最多两次且不许同形。第五部分是词库，其中「语气词每条最多一个」和「同句不许两次挺X的」两条密度限制是硬的。

4. `references/alive-dialogue.md`：活人感，全篇最长的一份。六节，前四节按作用范围从小到大排，后两节是触发式的，写到那个场景才执行：

    - **一、一句话**：两类句子（信息句／反应句）、旁白腔、文案空话、类比说完就停、句末不许解释因果。开头「两类句子」最要紧——「换个场景还成立就删掉」这条原则在提示词里出现过五次，反应句（啊？／真的假的／哈哈哈哈哈／草）按它全该删，删完对话就冷了。这一节给它划边界，后面五处一个字都不用改。
    - **二、一条消息**：长度、开头的停顿、中间怎么讲（把自己放进去／消息来源要具体／引入新信息的词）、结尾怎么收（附一句感受／不许用问句）。长度只说要拉开差距，几条长几条短跟着 `user.md` 的 `{answer_length}` 走，两处都写会跟 P1 那种全程短句的档位打架。
    - **三、一段对话**：第一轮先接住对方那件事、中间轮接球之后要有具体角度、最后一轮、整段的密度（「我跟你说」三个变体合起来一次）。
    - **四、user 那半边**：说话的三个来源、中间轮不许纯捧哏（user 第一轮和纯反应句那一轮除外）、怎么打字（可以只发情绪、可以没头没尾、细节要具体到离谱、不许每段都用「前两天+完整背景」开场）。
    - **五、对方在倾诉时**：四条禁令加三组违规／合格对照。
    - **六、自己不知道时**：说法词表加猜测的口吻。`knowledge-honesty.md` 的三档表指向这一节。
...
7. `prompts/user.md`：user 的骨架，九个占位符 `{domain}` `{keyword}` `{opinion}` `{n}` `{points}` `{first_id}` `{min_turns}` `{max_turns}` `{answer_length}`。内容顺序是：这一批哪些讨论点 → 讨论点只交代这段聊什么，user 的第一句台词由模型自己编 → 轮数区间和四条注水禁令 → 纯反应的批级要求（至少一半的段落有一轮纯反应，落在不同轮次）→ 长度形状 → 输出格式。
...
16. `scripts/check.py`：质检落盘后的 JSON，判的是内容好不好，跟结构无关。词表从 `references/words.md` 抄过来，改一处得改另一处，文件开头写着这条。
    ...
    硬失败（进不了 `dialogues`）：禁词、企业黑话、硬停词、模型路标、AI 自称、翻案腔 8 条正则、名词化 5 条正则、破折号、提示性冒号、语气词超限（单条最多一个，含两个以上长句时可到两个）、assistant 问句结尾、单条超过 `profile_max_chars`、占位符（`XX`、方括号花括号、某某张三李四）、半角标点、「哈哈」恰好两个、单独的「操」、一段里笑超过两次或两次同形。
