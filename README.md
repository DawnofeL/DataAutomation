...
4. `references/alive-dialogue.md`：活人感，全篇最长的一份。七节，第一节最要紧：
    ...
    - **两种句子**：把句子分成信息句和反应句。「换个场景还成立就删掉」这条只管信息句。这条原则在提示词里出现过五次，反应句（啊？／真的假的／哈哈哈哈哈／草）按它全该删，删完对话就冷了。这一节给它划边界，后面五处一个字都不用改。
    - 消息长度要拉开差距
    - 对话怎么推进：钩子要点名具体事物、首轮接球
    - **user 怎么写**：说话的三个来源、中间轮不许纯捧哏（纯反应那一轮除外）、怎么打字（可以只发情绪、可以没头没尾、细节要具体到离谱、不许每段都用「前两天+完整背景」开场）
    - 话怎么说：回答前有停顿、说完附一句感受、把自己放进去、说完就停
    - 情绪场景
    - 不许出现的说法：问句结尾、因果解释收尾、旁白腔、空洞接球、文案空话、类比后升华、收尾重复自己
    - 不知道的时候怎么退场
