...
4. `references/alive-dialogue.md`：活人感，全篇最长的一份。七节，第一节最要紧：

    - **反应句**：把句子分成信息句和反应句。「换个场景还成立就删掉」这条只管信息句。这条原则在提示词里出现过五次，反应句（啊？／真的假的／哈哈哈哈哈／草）按它全该删，删完对话就冷了。这一节给它划边界，后面五处一个字都不用改。
    - 消息长度要拉开差距。几条长几条短不写死在这儿，跟着 `user.md` 的 `{answer_length}` 走，两处都写会跟 P1 那种全程短句的档位打架。
    - **user 怎么写**：说话的三个来源、中间轮不许纯捧哏（user 第一轮和纯反应句那一轮除外）、怎么打字（可以只发情绪、可以没头没尾、细节要具体到离谱、不许每段都用「前两天+完整背景」开场）
    - 话怎么说：首轮先接住对方那件事、回答前有停顿、说完附一句感受、把自己放进去、消息来源要具体、引入新信息的词、「我跟你说」一段最多一次
    - 情绪场景：四条禁令加三组违规／合格对照
    - 不许出现的说法：问句结尾、因果解释收尾、旁白腔、空洞接球、文案空话、类比后升华、收尾
    - 不知道的时候怎么说
