...
16. `scripts/check.py`：质检落盘后的 JSON，判的是内容好不好，跟结构无关。词表从 `references/words.md` 抄过来，改一处得改另一处，文件开头写着这条。
    ...
    硬失败（进不了 `dialogues`）：禁词、企业黑话、硬停词、模型路标、AI 自称、翻案腔 8 条正则、名词化 5 条正则、破折号、提示性冒号、语气词超限（单条最多一个，含两个以上长句时可到两个）、assistant 问句结尾、单条超过 `profile_max_chars`、占位符（`XX`、方括号花括号、某某张三李四）、英文弯引号。
